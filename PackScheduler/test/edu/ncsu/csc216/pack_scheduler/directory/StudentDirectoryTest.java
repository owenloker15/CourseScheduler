...
public class StudentDirectoryTest {
	...
	private final String nonrealTestFile = "test-files/nonreal_student_records.txt";
	/** Test first name */
	...
	public void testLoadStudentsFromFile() {
		...
		try {
			sd2.loadStudentsFromFile(nonrealTestFile);
			fail("loadStudentsFromFile did not recognize that the student file does not exist");
		} catch (IllegalArgumentException e) {
			assertEquals(sd2.getStudentDirectory().length, 0);
		}
	}
	...
	public void testAddStudent() {
		...
		//Test invalid Student (null pw, null repeat pw, empty pw, empty repeat pw)
		try {
			sd.addStudent(FIRST_NAME, LAST_NAME, ID, EMAIL, null, PASSWORD, MAX_CREDITS);
			fail();
		} catch (IllegalArgumentException e) {
			String [][] directory = sd.getStudentDirectory();
		...
		// Test invalid Student (pw mismatch)
		try {
			sd.addStudent(FIRST_NAME, LAST_NAME, ID, EMAIL, PASSWORD, "abc", MAX_CREDITS);
			fail();
		} catch (IllegalArgumentException e) {
			String [][] directory = sd.getStudentDirectory();
			assertEquals(0, directory.length);
		...
		//Test valid Student (trigger too few credits fail-safe)
		sd.addStudent(FIRST_NAME, LAST_NAME, ID, EMAIL, PASSWORD, PASSWORD, 1);
		...
		// Test non-duplicate student add
		sd.addStudent(FIRST_NAME, LAST_NAME, "abcool5", EMAIL, PASSWORD, PASSWORD, MAX_CREDITS);
		studentDirectory = sd.getStudentDirectory();
	...
	/**
	 * Tests saveStudentDirectory(), specifically ensuring that it throws an exception when attempting to write somewhere it does not have permission
	 */
	@Test
